...
void RFM69_ATC::sendACK(const void* buffer, uint8_t bufferSize) {
  ...
  while (!canSend() && Timer.millis() - now < RF69_CSMA_LIMIT_MS) receiveDone();
...
void RFM69_ATC::sendFrame(uint8_t toAddress, const void* buffer, uint8_t bufferSize, bool requestACK, bool sendACK, bool sendRSSI, int16_t lastRSSI) {
  ...
  if (sendACK) {                   // TomWS1: adding logic to return ACK_RSSI if requested
    ...
    if (sendRSSI) {
      SPI.transfer(abs16(lastRSSI)); //RSSI dBm is negative expected between [-100 .. -20], convert to positive and pass along as single extra header byte
  ...
  while ((RF69_DIO0_PIN * RF69_DIO0_BIT) == 0 && Timer.millis() - txStart < RF69_TX_LIMIT_MS); // wait for DIO0 to turn HIGH signalling transmission finish
